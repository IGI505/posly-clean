...
class SalesReturnController extends Controller
{
    ...
    public function show(Request $request, $id)
    {
		...
		if ($user_auth->can('sale_returns_view_all') || $user_auth->can('sale_returns_view_own')){
            ...
            if($Sale_Return->discount_type == 'fixed'){
                $return_details['discount']           = $this->render_price_with_symbol_placement(number_format($Sale_Return->discount, 3, '.', ','));
            }else{
                $return_details['discount']           = $this->render_price_with_symbol_placement(number_format($Sale_Return->discount_percent_total, 3, '.', ',')) .' '.'('.$Sale_Return->discount .' '.'%)';
            }

            $return_details['shipping']       = $this->render_price_with_symbol_placement(number_format($Sale_Return->shipping, 3, '.', ','));
            $return_details['tax_rate']       = $Sale_Return->tax_rate;
            $return_details['TaxNet']         = $this->render_price_with_symbol_placement(number_format($Sale_Return->TaxNet, 3, '.', ','));
            ...
            $return_details['GrandTotal']     = $this->render_price_with_symbol_placement(number_format($Sale_Return->GrandTotal, 3, '.', ','));
            $return_details['paid_amount']    = $this->render_price_with_symbol_placement(number_format($Sale_Return->paid_amount, 3, '.', ','));
            $return_details['due']            = $this->render_price_with_symbol_placement(number_format($Sale_Return->GrandTotal - $Sale_Return->paid_amount, 3, '.', ','));
    ...
    public function Return_pdf(Request $request, $id)
    {
        ...
        foreach ($Sale_Return['details'] as $detail) {
            ...
            $data['discount']   = $detail->discount;number_format($detail->discount, 3, '.', '');
