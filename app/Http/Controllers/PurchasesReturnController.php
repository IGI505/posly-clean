...
class PurchasesReturnController extends Controller
{
		...
		if ($user_auth->can('purchase_returns_view_all') || $user_auth->can('purchase_returns_view_own')){
            ...
            if ($request->ajax()) {
                ...
                ->addColumn('GrandTotal', function($row){
                    return $this->render_price_with_symbol_placement(number_format($row->GrandTotal, 3, '.', ','));
                })
                ->addColumn('paid_amount', function($row){
                    return $this->render_price_with_symbol_placement(number_format($row->paid_amount, 3, '.', ','));
                })
                ->addColumn('due', function($row){
                    return $this->render_price_with_symbol_placement(number_format($row->GrandTotal - $row->paid_amount, 3, '.', ','));
                })
    ...
    public function show(Request $request, $id)
    {
        ...
        if ($user_auth->can('purchase_returns_view_all') || $user_auth->can('purchase_returns_view_own')){
            ...
            if($Purchase_Return->discount_type == 'fixed'){
                $return_details['discount']           = $this->render_price_with_symbol_placement(number_format($Purchase_Return->discount, 3, '.', ','));
            }else{
                $return_details['discount']           = $this->render_price_with_symbol_placement(number_format($Purchase_Return->discount_percent_total, 3, '.', ',')) .' '.'('.$Purchase_Return->discount .' '.'%)';
            }

            $return_details['shipping']       = $this->render_price_with_symbol_placement(number_format($Purchase_Return->shipping, 3, '.', ','));
            $return_details['tax_rate']       = $Purchase_Return->tax_rate;
            $return_details['TaxNet']         = $this->render_price_with_symbol_placement(number_format($Purchase_Return->TaxNet, 3, '.', ','));
            ...
            $return_details['GrandTotal']     = $this->render_price_with_symbol_placement(number_format($Purchase_Return->GrandTotal, 3, '.', ','));
            $return_details['paid_amount']    = $this->render_price_with_symbol_placement(number_format($Purchase_Return->paid_amount, 3, '.', ','));
            $return_details['due']            = $this->render_price_with_symbol_placement(number_format($Purchase_Return->GrandTotal - $Purchase_Return->paid_amount , 3, '.', ','));
    ...
    public function getNumberOrder()
    {
     ...
     public function Return_pdf(Request $request, $id)
     {
         ...
         $return_details['TaxNet']           = $this->render_price_with_symbol_placement(number_format($PurchaseReturn->TaxNet, 3, '.', ','));
         $return_details['discount']         = $this->render_price_with_symbol_placement(number_format($PurchaseReturn->discount, 3, '.', ','));
         $return_details['shipping']         = $this->render_price_with_symbol_placement(number_format($PurchaseReturn->shipping, 3, '.', ','));
         ...
         $return_details['GrandTotal']       = $this->render_price_with_symbol_placement(number_format($PurchaseReturn->GrandTotal, 3, '.', ','));
         $return_details['paid_amount']      = $this->render_price_with_symbol_placement(number_format($PurchaseReturn->paid_amount, 3, '.', ','));
         $return_details['due']              = $this->render_price_with_symbol_placement(number_format($PurchaseReturn->GrandTotal - $PurchaseReturn->paid_amount, 3, '.', ','));
