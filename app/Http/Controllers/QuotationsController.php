...
class QuotationsController extends Controller
{
    ...
    public function get_quotations_datatable(Request $request)
    {
        ...
        if (!$user_auth->can('quotations_view_all') && !$user_auth->can('quotations_view_own')){
            ...
            foreach ($quotations as $quotation) {
                ...
                $item['GrandTotal']      = $this->render_price_with_symbol_placement(number_format($quotation->GrandTotal, 3, '.', ','));
		...
		if ($user_auth->can('quotation_details')){
            ...
            $quotation_data = Quotation::with('details.product.unitSale')
                ...
                if($quotation_data->discount_type == 'fixed'){
                    $quote['discount']  = $this->render_price_with_symbol_placement(number_format($quotation_data->discount, 3, '.', ','));
                }else{
                    $quote['discount']  = $this->render_price_with_symbol_placement(number_format($quotation_data->discount_percent_total, 3, '.', ',')) .' '.'('.$quotation_data->discount .' '.'%)';
                }

                $quote['shipping']     = $this->render_price_with_symbol_placement(number_format($quotation_data->shipping, 3, '.', ','));
                $quote['tax_rate']     = $quotation_data->tax_rate;
                $quote['TaxNet']       = $this->render_price_with_symbol_placement(number_format($quotation_data->TaxNet, 3, '.', ','));
                ...
                $quote['GrandTotal']   = $this->render_price_with_symbol_placement(number_format($quotation_data->GrandTotal, 3, '.', ','));
    ...
    public function getNumberOrder()
    {
     ...
     public function Quotation_pdf(Request $request, $id)
     {
         ...
         $quote['TaxNet']       = $this->render_price_with_symbol_placement(number_format($Quotation->TaxNet, 3, '.', ','));

        if($Quotation->discount_type == 'fixed'){
            $quote['discount']  = $this->render_price_with_symbol_placement(number_format($Quotation->discount, 3, '.', ','));
        }else{
            $quote['discount']  = $this->render_price_with_symbol_placement(number_format($Quotation->discount_percent_total, 3, '.', ',')) .' '.'('.$Quotation->discount .' '.'%)';
        }

         $quote['shipping']   = $this->render_price_with_symbol_placement(number_format($Quotation->shipping, 3, '.', ','));
         $quote['statut']     = $Quotation->statut;
         $quote['Ref']        = $Quotation->Ref;
         $quote['date']       = Carbon::parse($Quotation->date)->format('d-m-Y H:i');
         $quote['GrandTotal'] = $this->render_price_with_symbol_placement(number_format($Quotation->GrandTotal, 3, '.', ','));
    ...
    public function SendEmail(Request $request)
    {
        ...
        $currency = $helpers->Get_Currency();
         ...
         $total_amount = $this->render_price_with_symbol_placement(number_format($quotation->GrandTotal, 3, '.', ','));
