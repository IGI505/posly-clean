...
class SalesController extends Controller
{
    ...
    public function get_sales_datatable(Request $request)
    {
        ...
        if (!$user_auth->can('sales_view_all') && !$user_auth->can('sales_view_own')){
            ...
            foreach ($sales as $sale) {
                ...
                $item['GrandTotal']     = $this->render_price_with_symbol_placement(number_format($sale->GrandTotal, 3, '.', ','));
                $item['paid_amount']    = $this->render_price_with_symbol_placement(number_format($sale->paid_amount, 3, '.', ','));
                $item['due']            = $this->render_price_with_symbol_placement(number_format($sale->GrandTotal - $sale->paid_amount, 3, '.', ','));
    ...
    public function show($id)
    {
		...
		if ($user_auth->can('sales_details')){
            ...
            if($sale_data->discount_type == 'fixed'){
                $sale_details['discount']           = $this->render_price_with_symbol_placement(number_format($sale_data->discount, 3, '.', ','));
            }else{
                $sale_details['discount']           = $this->render_price_with_symbol_placement(number_format($sale_data->discount_percent_total, 3, '.', ',')) .' '.'('.$sale_data->discount .' '.'%)';
            }

            $sale_details['shipping']               = $this->render_price_with_symbol_placement(number_format($sale_data->shipping, 3, '.', ','));
            $sale_details['tax_rate']               = $sale_data->tax_rate;
            $sale_details['TaxNet']                 = $this->render_price_with_symbol_placement(number_format($sale_data->TaxNet, 3, '.', ','));
            ...
            $sale_details['GrandTotal']             = $this->render_price_with_symbol_placement(number_format($sale_data->GrandTotal, 3, '.', ','));
            $sale_details['paid_amount']            = $this->render_price_with_symbol_placement(number_format($sale_data->paid_amount, 3, '.', ','));
            $sale_details['due']                    = $this->render_price_with_symbol_placement(number_format($sale_data->GrandTotal - $sale_data->paid_amount, 3, '.', ','));
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        $details = array();
         ...
         $item['Ref']                    = $sale->Ref;
         $item['date']                   = Carbon::parse($sale->date)->format('d-m-Y H:i');
         $item['shipping']               = $this->render_price_with_symbol_placement(number_format($sale->shipping, 3, '.', ','));

        if($sale->discount_type == 'fixed'){
            $item['discount']           = $this->render_price_with_symbol_placement(number_format($sale->discount, 3, '.', ','));
        }else{
            $item['discount']           = $this->render_price_with_symbol_placement(number_format($sale->discount_percent_total, 3, '.', ',')) .'('.$sale->discount .' '.'%)';
        }

         $item['taxe']                   = $this->render_price_with_symbol_placement(number_format($sale->TaxNet, 3, '.', ','));
     ...
     public function Sale_PDF(Request $request, $id)
     {
         ...
         $sale['tax_rate']               = number_format($sale_data->tax_rate, 3, '.', ' ');
         $sale['TaxNet']                 = $this->render_price_with_symbol_placement(number_format($sale_data->TaxNet, 3, '.', ','));

        if($sale_data->discount_type == 'fixed'){
            $sale['discount']           = $this->render_price_with_symbol_placement(number_format($sale_data->discount, 3, '.', ','));
        }else{
            $sale['discount']           = $this->render_price_with_symbol_placement(number_format($sale_data->discount_percent_total, 3, '.', ',')) .' '.'('.$sale_data->discount .' '.'%)';
        }

         $sale['shipping']               = $this->render_price_with_symbol_placement(number_format($sale_data->shipping, 3, '.', ','));
         $sale['statut']                 = $sale_data->statut;
         $sale['Ref']                    = $sale_data->Ref;
         $sale['date']                   = Carbon::parse($sale_data->date)->format('d-m-Y H:i');
         $sale['GrandTotal']             = $this->render_price_with_symbol_placement(number_format($sale_data->GrandTotal, 3, '.', ','));
         $sale['paid_amount']            = $this->render_price_with_symbol_placement(number_format($sale_data->paid_amount, 3, '.', ','));
         $sale['due']                    = $this->render_price_with_symbol_placement(number_format($sale_data->GrandTotal - $sale_data->paid_amount, 3, '.', ','));
            ...
            $unit = Unit::where('id', $detail->sale_unit_id)->first();
             if ($detail->product_variant_id) {
                ...
                $data['name'] = '['.$productsVariants->name . '] ' . $detail['product']['name'];
                 ...
                 $data['detail_id'] = $detail_id += 1;
                 $data['quantity'] = number_format($detail->quantity, 3, '.', '');
                 $data['total'] = number_format($detail->total, 3, '.', ' ');
                 $data['unitSale'] = $unit?$unit->ShortName:'';
                 $data['price'] = number_format($detail->price, 3, '.', ' ');

             if ($detail->discount_method == '2') {
                 $data['DiscountNet'] = number_format($detail->discount, 3, '.', '');
             } else {
                 $data['DiscountNet'] = number_format($detail->price * $detail->discount / 100, 3, '.', '');
             }

             $tax_price = $detail->TaxNet * (($detail->price - $data['DiscountNet']) / 100);
             $data['Unit_price'] = number_format($detail->price, 3, '.', '');
             $data['discount'] = number_format($detail->discount, 3, '.', '');

             if ($detail->tax_method == '1') {
                 $data['Net_price'] = $detail->price - $data['DiscountNet'];
                 $data['taxe'] = number_format($tax_price, 3, '.', '');
             } else {
                 $data['Net_price'] = ($detail->price - $data['DiscountNet']) / (($detail->TaxNet / 100) + 1);
                 $data['taxe'] = number_format($detail->price - $data['Net_price'] - $data['DiscountNet'], 3, '.', '');
             }
    ...
    public function Send_Email(Request $request)
    {
         ...
         $total_amount = $this->render_price_with_symbol_placement(number_format($sale->GrandTotal, 3, '.', ','));
         $paid_amount  = $this->render_price_with_symbol_placement(number_format($sale->paid_amount, 3, '.', ','));
         $due_amount   = $this->render_price_with_symbol_placement(number_format($sale->GrandTotal - $sale->paid_amount, 3, '.', ','));
