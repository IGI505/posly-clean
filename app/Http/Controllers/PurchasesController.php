...
class PurchasesController extends Controller
{
    ...
    public function get_purchases_datatable(Request $request)
    {
        ...
        if (!$user_auth->can('purchases_view_all') && !$user_auth->can('purchases_view_own')){
            ...
            foreach ($purchases as $purchase) {
                ...
                $item['GrandTotal']     = $this->render_price_with_symbol_placement(number_format($purchase->GrandTotal, 3, '.', ','));
                $item['paid_amount']    = $this->render_price_with_symbol_placement(number_format($purchase->paid_amount, 3, '.', ','));
                $item['due']            = $this->render_price_with_symbol_placement(number_format($purchase->GrandTotal - $purchase->paid_amount, 3, '.', ','));
    ...
    public function show($id)
    {
		...
		if ($user_auth->can('purchases_details')){
            ...
            if($purchase->discount_type == 'fixed'){
                $purchase_data['discount']           = $this->render_price_with_symbol_placement(number_format($purchase->discount, 3, '.', ','));
            }else{
                $purchase_data['discount']           = $this->render_price_with_symbol_placement(number_format($purchase->discount_percent_total, 3, '.', ',')) .' '.'('.$purchase->discount .' '.'%)';
            }

            $purchase_data['shipping']               = $this->render_price_with_symbol_placement(number_format($purchase->shipping, 3, '.', ','));
            $purchase_data['tax_rate']               = $purchase->tax_rate;
            $purchase_data['TaxNet']                 = $this->render_price_with_symbol_placement(number_format($purchase->TaxNet, 3, '.', ','));
            ...
            $purchase_data['GrandTotal']             = $this->render_price_with_symbol_placement(number_format($purchase->GrandTotal, 3, '.', ','));
            $purchase_data['paid_amount']            = $this->render_price_with_symbol_placement(number_format($purchase->paid_amount, 3, '.', ','));
            $purchase_data['due']                    = $this->render_price_with_symbol_placement(number_format($purchase->GrandTotal - $purchase->paid_amount, 3, '.', ','));
    ...
    public function Purchase_pdf(Request $request, $id)
    {
        ...
        $purchase['tax_rate']       = number_format($Purchase_data->tax_rate, 3, '.', '');
        $purchase['TaxNet']         = $this->render_price_with_symbol_placement(number_format($Purchase_data->TaxNet, 3, '.', ','));

        if($Purchase_data->discount_type == 'fixed'){
            $purchase['discount']           = $this->render_price_with_symbol_placement(number_format($Purchase_data->discount, 3, '.', ','));
        }else{
            $purchase['discount']           = $this->render_price_with_symbol_placement(number_format($Purchase_data->discount_percent_total, 3, '.', ',')) .' '.'('.$Purchase_data->discount .' '.'%)';
        }

        $purchase['shipping']               = $this->render_price_with_symbol_placement(number_format($Purchase_data->shipping, 3, '.', ','));
        $purchase['Ref']                    = $Purchase_data->Ref;
        $purchase['date']                   = Carbon::parse($Purchase_data->date)->format('d-m-Y H:i');
        $purchase['GrandTotal']             = $this->render_price_with_symbol_placement(number_format($Purchase_data->GrandTotal, 3, '.', ','));
        $purchase['paid_amount']            = $this->render_price_with_symbol_placement(number_format($Purchase_data->paid_amount, 3, '.', ','));
        $purchase['due']                    = $this->render_price_with_symbol_placement(number_format($Purchase_data->GrandTotal- $Purchase_data->paid_amount, 3, '.', ','));
        ...
        foreach ($Purchase_data['details'] as $detail) {
            ...
            if ($detail->product_variant_id) {
                ...
                $data['unit_purchase'] = $unit->ShortName;
                $data['cost'] = number_format($detail->cost, 3, '.', ',');

            if ($detail->discount_method == '2') {
                $data['DiscountNet'] = number_format($detail->discount, 3, '.', '');
            } else {
                $data['DiscountNet'] = number_format($detail->cost * $detail->discount / 100, 3, '.', '');
            }

            $tax_cost = $detail->TaxNet * (($detail->cost - $data['DiscountNet']) / 100);
            $data['Unit_cost'] = number_format($detail->cost, 3, '.', '');
            $data['discount'] = number_format($detail->discount, 3, '.', '');

            if ($detail->tax_method == '1') {

                $data['Net_cost'] = $detail->cost - $data['DiscountNet'];
                $data['taxe'] = number_format($tax_cost, 3, '.', '');
            } else {
                $data['Net_cost'] = ($detail->cost - $data['DiscountNet']) / (($detail->TaxNet / 100) + 1);
                $data['taxe'] = number_format($detail->cost - $data['Net_cost'] - $data['DiscountNet'], 3, '.', '');
            }
    ...
    public function Send_Email(Request $request)
    {
        ...
        $total_amount = $this->render_price_with_symbol_placement(number_format($purchase->GrandTotal, 3, '.', ','));
        $paid_amount  = $this->render_price_with_symbol_placement(number_format($purchase->paid_amount, 3, '.', ','));
        $due_amount   = $this->render_price_with_symbol_placement(number_format($purchase->GrandTotal - $purchase->paid_amount, 3, '.', ','));
