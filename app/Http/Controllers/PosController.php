...
//            $order->Ref = 'SO-' . date("Ymd") . '-'. date("his");
        ...
        foreach ($product_warehouse_data as $product_warehouse) {
            ...
            if ($product_warehouse['product']->TaxNet !== 0.0) {
                ...
                if ($product_warehouse['product']->tax_method == '1') {
                    ...
                    $item['Net_price'] = $this->render_price_with_symbol_placement(number_format($price + $tax_price, 3, '.', ','));

                    // Inxclusive
                } else {
                    $item['Net_price'] = $this->render_price_with_symbol_placement(number_format($price, 3, '.', ','));
                }
            } else {
                $item['Net_price'] = $this->render_price_with_symbol_placement(number_format($price, 3, '.', ','));
            }
    ...
    public function Print_Invoice_POS(Request $request, $id)
    {
        ...
        if ($user_auth->can('pos')) {
            ...
            $item['time'] = Carbon::parse($sale->date)->format('H:i'); // Adjust to use the same date field for time if needed
            ...
            foreach ($sale['details'] as $detail) {
                ...
                $data['price'] = $this->render_price_with_symbol_placement(number_format($detail->price, 3, '.', ','));
                $data['total'] = $this->render_price_with_symbol_placement(number_format($detail->total, 3, '.', ','));
            ...
            foreach ($payments as $payment) {
                ...
                $payment_data['montant'] = $this->render_price_with_symbol_placement(number_format($payment->montant, 3, '.', ','));

                $payments_details[] = $payment_data;
            }

            $settings = Setting::whereNull('deleted_at')->first();
            $pos_settings = PosSetting::whereNull('deleted_at')->first();
